...
export default class CameraController {
	...
	constructor(camera, domElement = document.body) {
		...
		let dX = this.camera.position[0];
		let dY = this.camera.position[1];
		let dZ = this.camera.position[2];
		let radius = Math.sqrt(dX * dX + dY * dY + dZ * dZ);
		let theta = Math.atan2(this.camera.position[0], this.camera.position[2]); // equator angle around y-up axis
		let phi = Math.acos(clamp(this.camera.position[1] / radius, -1, 1)); // polar angle
		this._spherical = {
			radius: radius,
			theta: theta,
			phi: phi
