...
`;
  ...
  var VERTEX_SHADER_BASE = "#define GLSLIFY 1\nattribute vec4 position;\n#ifdef INCLUDE_UVS\nattribute vec2 uv;\n#endif\nvarying vec2 v_uv;void main(){gl_Position=projectionMatrix*viewMatrix*modelMatrix*position;\n#ifdef INCLUDE_UVS\nv_uv=uv;\n#endif\n}"; // eslint-disable-line

  var FRAGMENT_SHADER_BASE = "precision highp float;\n#define GLSLIFY 1\nuniform sampler2D texture;\n#ifdef INCLUDE_UVS\nvarying vec2 v_uv;\n#endif\n#ifdef USE_FXAA\nuniform vec2 resolution;void texcoords(vec2 fragCoord,vec2 resolution,out vec2 v_rgbNW,out vec2 v_rgbNE,out vec2 v_rgbSW,out vec2 v_rgbSE,out vec2 v_rgbM){vec2 inverseVP=1.0/resolution.xy;v_rgbNW=(fragCoord+vec2(-1.0,-1.0))*inverseVP;v_rgbNE=(fragCoord+vec2(1.0,-1.0))*inverseVP;v_rgbSW=(fragCoord+vec2(-1.0,1.0))*inverseVP;v_rgbSE=(fragCoord+vec2(1.0,1.0))*inverseVP;v_rgbM=vec2(fragCoord*inverseVP);}\n#ifndef FXAA_REDUCE_MIN\n#define FXAA_REDUCE_MIN (1.0/ 128.0)\n#endif\n#ifndef FXAA_REDUCE_MUL\n#define FXAA_REDUCE_MUL (1.0 / 8.0)\n#endif\n#ifndef FXAA_SPAN_MAX\n#define FXAA_SPAN_MAX 8.0\n#endif\nvec4 fxaa(sampler2D tex,vec2 fragCoord,vec2 resolution,vec2 v_rgbNW,vec2 v_rgbNE,vec2 v_rgbSW,vec2 v_rgbSE,vec2 v_rgbM){vec4 color;mediump vec2 inverseVP=vec2(1.0/resolution.x,1.0/resolution.y);vec3 rgbNW=texture2D(tex,v_rgbNW).xyz;vec3 rgbNE=texture2D(tex,v_rgbNE).xyz;vec3 rgbSW=texture2D(tex,v_rgbSW).xyz;vec3 rgbSE=texture2D(tex,v_rgbSE).xyz;vec4 texColor=texture2D(tex,v_rgbM);vec3 rgbM=texColor.xyz;vec3 luma=vec3(0.299,0.587,0.114);float lumaNW=dot(rgbNW,luma);float lumaNE=dot(rgbNE,luma);float lumaSW=dot(rgbSW,luma);float lumaSE=dot(rgbSE,luma);float lumaM=dot(rgbM,luma);float lumaMin=min(lumaM,min(min(lumaNW,lumaNE),min(lumaSW,lumaSE)));float lumaMax=max(lumaM,max(max(lumaNW,lumaNE),max(lumaSW,lumaSE)));mediump vec2 dir;dir.x=-((lumaNW+lumaNE)-(lumaSW+lumaSE));dir.y=((lumaNW+lumaSW)-(lumaNE+lumaSE));float dirReduce=max((lumaNW+lumaNE+lumaSW+lumaSE)*(0.25*FXAA_REDUCE_MUL),FXAA_REDUCE_MIN);float rcpDirMin=1.0/(min(abs(dir.x),abs(dir.y))+dirReduce);dir=min(vec2(FXAA_SPAN_MAX,FXAA_SPAN_MAX),max(vec2(-FXAA_SPAN_MAX,-FXAA_SPAN_MAX),dir*rcpDirMin))*inverseVP;vec3 rgbA=0.5*(texture2D(tex,fragCoord*inverseVP+dir*(1.0/3.0-0.5)).xyz+texture2D(tex,fragCoord*inverseVP+dir*(2.0/3.0-0.5)).xyz);vec3 rgbB=rgbA*0.5+0.25*(texture2D(tex,fragCoord*inverseVP+dir*-0.5).xyz+texture2D(tex,fragCoord*inverseVP+dir*0.5).xyz);float lumaB=dot(rgbB,luma);if((lumaB<lumaMin)||(lumaB>lumaMax)){color=vec4(rgbA,texColor.a);}else{color=vec4(rgbB,texColor.a);}return color;}vec4 applyFXAA(sampler2D tex,vec2 fragCoord,vec2 resolution){mediump vec2 v_rgbNW;mediump vec2 v_rgbNE;mediump vec2 v_rgbSW;mediump vec2 v_rgbSE;mediump vec2 v_rgbM;texcoords(fragCoord,resolution,v_rgbNW,v_rgbNE,v_rgbSW,v_rgbSE,v_rgbM);return fxaa(tex,fragCoord,resolution,v_rgbNW,v_rgbNE,v_rgbSW,v_rgbSE,v_rgbM);}\n#endif\n#ifdef IS_DEPTH_TEXTURE\nconst float near_plane=0.1;const float far_plane=100.0;float LinearizeDepth(float depth){float z=depth*2.0-1.0;return(2.0*near_plane*far_plane)/(far_plane+near_plane-z*(far_plane-near_plane));}\n#endif\nvoid main(){\n#ifdef USE_FXAA\nvec2 fragCoord=v_uv*resolution;gl_FragColor=applyFXAA(texture,fragCoord,resolution);\n#else\n#ifdef IS_DEPTH_TEXTURE\nfloat depthValue=texture2D(texture,v_uv).r;float debugDepth=LinearizeDepth(depthValue)/far_plane;gl_FragColor=vec4(vec3(debugDepth),1.0);\n#else\ngl_FragColor=texture2D(texture,v_uv);\n#endif\n#endif\n}"; // eslint-disable-line

  var VERTEX_SHADER_GBUFFER = "#define GLSLIFY 1\nattribute vec4 position;attribute vec3 color;attribute vec3 normal;attribute mat4 instanceModelMatrix;varying vec3 v_color;varying vec3 v_normal;varying vec3 v_position;void main(){vec4 worldPosition=modelMatrix*instanceModelMatrix*position;gl_Position=projectionMatrix*viewMatrix*worldPosition;v_color=color;v_normal=(modelMatrix*vec4(normal,1.0)).xyz;v_position=worldPosition.xyz;}"; // eslint-disable-line

  var FRAGMENT_SHADER_GBUFFER = "#extension GL_EXT_draw_buffers : require\nprecision highp float;\n#define GLSLIFY 1\nvarying vec3 v_color;varying vec3 v_normal;varying vec3 v_position;void main(){gl_FragData[0]=vec4(v_position,0.0);gl_FragData[1]=vec4(normalize(v_normal.xyz),0.0);gl_FragData[2]=vec4(v_color,0.0);}"; // eslint-disable-line

  var FRAGMENT_SHADER_POINT_LIGHTING = "precision highp float;\n#define GLSLIFY 1\nstruct PointLightBase{vec3 shininessSpecularRadius;vec3 position;vec3 color;};uniform sampler2D positionTexture;uniform sampler2D normalTexture;uniform sampler2D colorTexture;uniform vec3 eyePosition;uniform vec2 resolution;uniform PointLightBase PointLight;void main(){vec2 fragCoord=gl_FragCoord.xy/resolution;vec3 position=texture2D(positionTexture,fragCoord).xyz;vec3 normal=normalize(texture2D(normalTexture,fragCoord).xyz);vec3 color=texture2D(colorTexture,fragCoord).xyz;vec4 baseColor=vec4(color,1.0);vec3 eyeDirection=normalize(eyePosition-position);vec3 lightVec=PointLight.position-position;float shininess=PointLight.shininessSpecularRadius.x;float lightR=PointLight.shininessSpecularRadius.z;float dist=distance(PointLight.position,position)*2.0;if(dist<lightR){float attenuation=dist/(1.0-(dist/lightR)*(dist/lightR));attenuation=attenuation/lightR+1.0;attenuation=1.0/(attenuation*attenuation);vec3 lightDirection=normalize(lightVec);vec3 reflectionDirection=reflect(-lightDirection,normal);float nDotL=max(dot(lightDirection,normal),0.0);vec3 diffuse=nDotL*PointLight.color;gl_FragColor=vec4(diffuse*PointLight.color*baseColor.rgb*attenuation*shininess,baseColor.a);}}"; // eslint-disable-line

  var FRAGMENT_SHADER_DIRECTIONAL_LIGHTING = "precision highp float;\n#define GLSLIFY 1\nuniform vec2 resolution;uniform sampler2D positionTexture;uniform sampler2D normalTexture;uniform sampler2D colorTexture;uniform vec3 lightDirection;uniform float lightFactor;void main(){vec2 fragCoord=gl_FragCoord.xy/resolution;vec3 normal=normalize(texture2D(normalTexture,fragCoord).xyz);vec3 color=texture2D(colorTexture,fragCoord).xyz;float light=dot(normal,lightDirection);gl_FragColor=vec4(color*light*lightFactor,1.0);}"; // eslint-disable-line
  ...
  {
    const { indices, vertices, normal } = index.createBox();
    const geometry = new Geometry(gl);
    ...
    for (let x = 0; x < BOXES_ROW_X_COUNT; x++) {
      for (let y = 0; y < BOXES_ROW_Z_COUNT; y++) {
        ...
        const translateVec = fromValues(posX, scaleY / 2, posZ);
        const matrix = create();
        translate(matrix, matrix, translateVec);
        scale(matrix, matrix, fromValues(1, scaleY, 1));
  ...
  {
    const { indices, vertices } = index.createPlane({
      width: innerWidth,
      height: innerHeight,
    });
    const geometry = new Geometry(gl);
