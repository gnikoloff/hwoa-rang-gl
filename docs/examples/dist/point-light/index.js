...
`;
	...
	const instanceMatrix = create$3();
	const translateVector = create$2();
	...
	const lightWorldPosition = create$2();
	set$1(lightWorldPosition, 0, 0, 0);

	const lightDirection = create$2();
	set$1(lightDirection, 0, 5, 5);
	normalize$1(lightDirection, lightDirection);

	{
	  const { indices, vertices, normal } = index.createSphere({
	    widthSegments: 20,
	    heightSegments: 20,
	  });
	  ...
	  for (let i = 0; i < BALLS_COUNT_SIZE; i++) {
	    for (let n = 0; n < BALLS_COUNT_SIZE; n++) {
	      for (let j = 0; j < BALLS_COUNT_SIZE; j++) {
	        ...
	        identity$1(instanceMatrix);
	        set$1(translateVector, x, y, z);
	        translate$1(instanceMatrix, instanceMatrix, translateVector);
	...
	{
	  const { vertices, indices } = index.createSphere({
	    radius: 0.1,
	  });
	  const geometry = new Geometry(gl);
	...
	function updateFrame(ts) {
	  ...
	  set$1(lightWorldPosition, 0, 0, Math.sin(ts * 0.8) * 8);
