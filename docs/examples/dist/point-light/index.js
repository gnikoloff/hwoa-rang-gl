...
`;
	...
	const instanceMatrix = create();
	const translateVector = create$1();
	...
	const lightWorldPosition = create$1();
	set(lightWorldPosition, 0, 0, 0);

	const lightDirection = create$1();
	set(lightDirection, 0, 5, 5);
	  ...
	  for (let i = 0; i < BALLS_COUNT_SIZE; i++) {
	    for (let n = 0; n < BALLS_COUNT_SIZE; n++) {
	      for (let j = 0; j < BALLS_COUNT_SIZE; j++) {
	        ...
	        set(translateVector, x, y, z);
	        translate(instanceMatrix, instanceMatrix, translateVector);
	...
	function updateFrame(ts) {
	  ...
	  set(lightWorldPosition, 0, 0, Math.sin(ts * 0.8) * 8);
